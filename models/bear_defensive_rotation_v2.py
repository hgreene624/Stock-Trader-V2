...
Bear market specialist that rotates among defensive assets with cash option.

Version 2 Improvements:
- Added SHY (cash proxy) to defensive universe - lets it compete in momentum ranking
- Shorter momentum period (20 vs 30 days) for faster response
- New cash_threshold parameter: if ALL assets < threshold, go 100% SHY
- Better handling of rising rate environments where bonds crash
...
- NEW: Goes to 100% SHY when all assets are below cash_threshold (-0.10)
- Rebalances every N days (default 10)

Design Rationale:
- In bear markets, capital preservation is paramount
- V2 recognizes that in rising rate environments, bonds can crash alongside equities
- SHY (1-3 month T-bills) provides true cash-like safety
- Faster momentum period allows quicker response to changing conditions
- Cash threshold provides circuit breaker when everything is falling
"""
...
class BearDefensiveRotation_v2(BaseModel):
    """
    Bear market defensive rotation v2 - rotate to "least bad" defensive assets with cash option.

    Only generates weights when equity_regime == 'bear'.
    V2 adds SHY to universe and cash threshold for true capital preservation.
    """

    def __init__(
        self,
        model_id: str = "BearDefensiveRotation_v2",
        defensive_assets: list[str] = None,
        momentum_period: int = 20,  # V2: Faster response (was 30)
        top_n: int = 2,
        min_momentum: float = -0.05,  # Can hold negative momentum assets
        cash_threshold: float = -0.10,  # V2: NEW - if all assets below this, go to SHY
        rebalance_days: int = 10
    ):
        """
        Initialize Bear Defensive Rotation Model v2.

        Args:
            model_id: Unique model identifier
            defensive_assets: List of defensive ETFs (default: XLU, XLP, TLT, GLD, UUP, SHY)
            momentum_period: Lookback period for momentum (default: 20 days - faster than v1)
            top_n: Number of top assets to hold (default: 2)
            min_momentum: Minimum momentum threshold (default: -0.05, can be negative)
            cash_threshold: NEW - If ALL assets below this, go 100% SHY (default: -0.10)
            rebalance_days: Days between rebalances (default: 10)
        """
        # V2: SHY added to defensive assets list - competes with others
        self.defensive_assets = defensive_assets or [
            ...
            'GLD',   # Gold
            'UUP',   # US Dollar Index
            'SHY'    # V2: Short-term Treasury (cash proxy) - NEW!
        ]

        self.cash_asset = 'SHY'  # V2: Explicit cash asset
        ...
        self.cash_threshold = cash_threshold  # V2: NEW parameter
    ...
    def generate_target_weights(self, context: Context) -> ModelOutput:
        ...
        V2: Includes cash threshold check for extreme conditions.
        ...
        if len(qualified_assets) > 0:
            ...
            # V2: Changed from TLT fallback to SHY for true safety
            weights[self.cash_asset] = 1.0
    ...
    def __repr__(self):
        return (
            f"BearDefensiveRotation_v2(model_id='{self.model_id}', "
