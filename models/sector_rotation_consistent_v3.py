...
class SectorRotationConsistent_v3(BaseModel):
    """
    Consistent yearly alpha with CRASH PROTECTION + DIP BUYING.

    V3 improvements over v2:
    - Fast crash detection (SPY -7% in 5 days OR VIX > 35)
    - Automatic defensive mode during crashes
    - Gradual scale-in during recovery (dip buying)
    - Don't try to optimize for black swans, just survive them

    Philosophy: Capital preservation > alpha during crashes
    """

    def __init__(
        self,
        model_id: str = "SectorRotationConsistent_v3",
